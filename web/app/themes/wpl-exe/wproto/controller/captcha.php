 ...
 **/
class wpl_exe_wp_captcha_controller extends wpl_exe_wp_core_controller {
	...
	public function __construct() {
		...
		// add a captcha field to the comment form
		// add_action( 'comment_form', array( $this, 'add_captcha_to_comments' ) );
	...
	/**
	 * Add captcha field to the comments
	 **/
	function add_captcha_to_comments() {
		global $wpl_exe_wp;
		...
		if( !is_user_logged_in() && $captcha_enabled ):
			...
			?>
			<p class="captcha">
				<?php $this->generate_captcha_phrase(); ?>
			</p>
			<?php
	...
	public function generate_captcha_phrase() {
		...
		<input type="text" min="0" size="2" class="wproto-captcha-input" name="wproto-captcha-value" value="" />';

		//available operations
		$captcha_diff = $wpl_exe_wp->get_option( 'captcha_difficult', 'general' );
	...
	*/
	...
	function add_comment_with_captcha( $comment ) {
		...
		$captcha_enabled = $wpl_exe_wp->get_option( 'comments_captcha_enabled', 'general' );
		$hide_from_logged = $wpl_exe_wp->get_option( 'comments_captcha_only_for_guests', 'general' );
		
		if( ((is_user_logged_in() && ! $hide_from_logged ) || !is_user_logged_in()) && $captcha_enabled ):
