	...
	var wprotoScreenPricingTables = {
		...
		**/
		build: function() {
			...
			$('#wproto-pricing-table-editor *[title]').tipsy({gravity: 's'});
		...
		events: function() {
			...
			$('.wproto-add-pricing-package').click( function() {
				...
				table.find('tbody tr.system-item').each( function() {
					...
					if( $(this).hasClass('item-currency') ) {
						html = '<td><input name="pt[features][currency][0][]" type="text" value="" /></td>';	
						$(html).insertBefore( $(this).find('td:last') );
					}
					
					if( $(this).hasClass('item-period') ) {
						html = '<td><input name="pt[features][period][0][]" type="text" value="" /></td>';	
						$(html).insertBefore( $(this).find('td:last') );
					}
			
					if( $(this).hasClass('item-details') ) {
						html = '<td><textarea name="pt[features][details][0][]"></textarea></td>';	
						$(html).insertBefore( $(this).find('td:last') );				
					}
					
					if( $(this).hasClass('item-rating') ) {
						html = '<td><input name="pt[features][rating][0][]" type="text" value="" /></td>';	
						$(html).insertBefore( $(this).find('td:last') );
					}
					
					if( $(this).hasClass('item-icon') ) {
						html = '<td><i data-name="" class="wproto-icon-holder icon-2x"></i> <a href="javascript:;" class="wproto-icon-chooser">' + wprotoVars.strSelectIcon + '</a><input type="hidden" value="" name="pt[features][icon][0][]" class="wproto-icon-holder-input" /></td>';	
						$(html).insertBefore( $(this).find('td:last') );
					}
