...
function add_meta_details() {
    ...
    if (! empty ( $videoID )) {
        ...
        if (! empty ( $video_count )) { 
          ...
          $swfPath      = FLICHE_VGALLERY_BASEURL . 'hdflvplayer' . DS . 'hdplayer.swf';
          /** Get video page URL */
          ...
          if (is_ssl () && $_SERVER ['SERVER_PORT'] == 443) {
            $output .= '<meta property="og:video:type" content="application/x-shockwave-flash" />
                <meta property="og:video" content="' . $swfPath . '?vid=' . $videoID . '&baserefW=' . FLICHE_VGALLERY_BASEURL . '&embedplayer=true" />
                <meta property="og:video:secure_url" content="' . $swfPath . '?vid=' . $videoID . '&baserefW=' . FLICHE_VGALLERY_BASEURL . '&embedplayer=true" />';
          }
...
/**
 * Function to include widget files
 *  
...
function includeWidgetfiles ( $fileName ) {   
    ...
    if (file_exists ( $widgetPath . DS . $fileName )) {
      ...
      include_once ( FLICHE_VGALLERY_BASEDIR . DS . $fileName);
    }  
}
/**
