 ...
 * @author user
 */
class Widget_FlicheVideoSearch_init extends WP_Widget {    
    /**
     * Search widget init function
     */
    function __construct() {
        /** Array to store search widget class name, description */
        $widget_ops = array ( 'classname' => 'Widget_FlicheVideoSearch_init ', 'description' => 'Search Videos Widget' );
        #$this->WP_Widget ( 'Widget_FlicheVideoSearch', 'Fliche Video Search', $widget_ops );
        parent::__construct ( 'Widget_FlicheVideoSearch', 'Fliche Video Search', $widget_ops );
    }
    ...
    function widget($args, $instance) {   
        ...
        $searchVal  = __ ( 'Video Search &hellip;', FLICHE_VGALLERY );        
        ...
        $div = '<div id="videos-search"  class="widget_search sidebar-wrap search-form-container "><h3 class="widget-title">' . $title . '</h3>
            <form role="search" method="get" id="search-form" class="search-form searchform clearfix" action="' . home_url ( '/' ) . '" >
              ...
              <input type="submit" class="search-submit submit" id="videosearchsubmit" value="' . __ ( 'Search', FLICHE_VGALLERY ) . '" />
          </form> </div>';
...
add_action ( 'widgets_init', create_function ( '', 'return register_widget("Widget_FlicheVideoSearch_init" );' ) ); 
?>