...
/**
 * Check FlicheVideoShortcodeView class exists
 */
if (! class_exists ( 'FlicheVideoShortcodeView' )) {
  /**
   * FlicheVideoShortcodeView class starts
   * 
   * @author user
   */
    class FlicheVideoShortcodeView extends FlicheVideoShortcodeController {
        /**
         * FlicheVideoShortcodeView constructor function
         */
        public function __construct() {
            ...
            $this->_contOBJ         = new FlicheVideoController ();
        ...
        function displayViewsPostedON ($show_added_on, $videogalleryviews, $no_views, $post_date, $hitcount){
          ...
          if (($this->_post_type === FLICHEVIDEOGALLERY || $this->_page_post_type === FLICHEVIDEOGALLERY ) && $show_added_on == 1 ) {
            /** Code to display posted on under player */
              $output .= '<div class="video-page-date"><strong>' . __ ( 'Posted&nbsp;on', FLICHE_VGALLERY ) . '&nbsp;:&nbsp;</strong><span>' . date_i18n ( get_option ( 'date_format' ), strtotime ( $post_date ) ) . '</span></div>';
          }
          /** Check views count is enabled */
          if ($videogalleryviews) {
            /** Display views count under player */
            $output .= '<div class="video-page-views"><strong>' . __ ( 'Views', FLICHE_VGALLERY ) . '&nbsp;:&nbsp;</strong><span>' . $hitcount . '</span></div>';
          }
          /** Return views and posted on content */ 
          return $output;
        }        
        
    /** FlicheVideoShortcodeView class ends */
    } 
/** Check FlicheVideoShortcodeView class exists if ends */
} else {
    /** Display message FlicheVideoShortcodeView exists */
    echo 'class FlicheVideoShortcodeView already exists';
}
...
class FlicheCategoryAdsenseDisplay extends FlicheVideoShortcodeView {
  ...
  function displayCategory( $vid ) {
    ...
    $output   .= '<div class="video-page-category"><strong>' . __ ( 'Category', FLICHE_VGALLERY ) . '&nbsp;:&nbsp;</strong>';
  ...
  function displayGoogleAdsense ($width, $vid, $videoid) {
    ...
    $output   .= '<div id="lightm"  style="' . $adstyle . 'height:76px;position:absolute;display:none;background:none !important;background-position: initial initial !important;background-repeat: initial initial !important;bottom: 60px;left: 50%;">
                        <span id="divimgm" >
                          <img alt="close" id="closeimgm" src="' . FLICHE_VGALLERY_BASEURL . '/images/close.png" style="z-index: 10000000;width:48px;height:12px;cursor:pointer;top:-12px;" onclick="googleclose();"  />
                        </span>
    ...
    $output   .= '<script type="text/javascript">
                        ...
                        </script> <script src="' . FLICHE_VGALLERY_BASEURL . 'js/googlead.js" type="text/javascript"></script>';
    return $output;
  }
}

/** Check FlicheSocialCommentView class exists */
if (! class_exists ( 'FlicheSocialCommentView' )) {
  /**
   * FlicheSocialCommentView class
   * 
   * @author user
   */
  class FlicheSocialCommentView extends FlicheCategoryAdsenseDisplay {
    /**
    ...
    public function displaySocialIcons ( $fbAPIKey, $videodescription, $video_thumb, $video_title ) {
      ...
      return '<div class="video-socialshare">
                          ...
                          <div class="floatleft ttweet" >
                              <a href="https://twitter.com/share" class="twitter-share-button" text="Twitter" data-count="none" data-url="' . $current_url . '" data-via="' . $blog_title . '" data-text="' . $video_title . '">' . __ ( 'Tweet', FLICHE_VGALLERY ) . '</a>
      ...
      if ($this->_post_type === FLICHEVIDEOGALLERY || $this->_page_post_type === FLICHEVIDEOGALLERY) {
        ...
        switch ($configXML->comment_option) {
          ...
          case 2 :
            ...
            $output .= '<style type="text/css">#respond,#comments #respond,#comments.comments-area, #disqus_thread, .comments-link{ display: none!important; } </style>';
            $output .= '<div class="clear"></div> <h2 class="related-videos">' . __ ( 'Post Your Comments', FLICHE_VGALLERY ) . '</h2> <div id="fb-root"></div>
          ...
          case 3 :
            ...
            $output .= '<div id="disqus_thread"></div>
                         ...
                         </script> <noscript>' . __ ( 'Please enable JavaScript to view the', FLICHE_VGALLERY ) . ' <a href="http://disqus.com/?ref_noscript">' . __ ( 'comments powered by Disqus.', FLICHE_VGALLERY ) . '</a> </noscript>
                         <a href="http://disqus.com" class="dsq-brlink">' . __ ( 'comments powered by', FLICHE_VGALLERY ) . ' <span class="logo-disqus">' . __ ( 'Disqus', FLICHE_VGALLERY ) . '</span></a>';
            break;
          default :
            break;
        }
