      ...
      function redirectGadsPage ( $statusFlag, $action, $type ) {
        ...
        */
        ...
        public function get_message() {
          ...
          if ($this->_add == '1') {
            $result = set_message( $adsType, __ ('Added', FLICHE_VGALLERY ));
          }
          /** Check adsense add action is unsuccess */
          if ($this->_add == '0') {
            $result = set_message( $adsType, __ ('Not Added', FLICHE_VGALLERY ));
          }
          /** Check adsense publish action is success */
          if($this->_status == '1') {
            $result = set_message( $adsType, __ ( 'Published', FLICHE_VGALLERY ));
          }
          /** Check adsense update action is success */
          if($this->_update == '1') {
            $result = set_message( $adsType, __ ('Updated', FLICHE_VGALLERY ));
          }
          /** Check adsense update action is unsuccess */
          if($this->_update == '0' ) {
            $result = set_message( $adsType, __ ('Not Updated', FLICHE_VGALLERY ));
          }
          /** Check adsense unpublish action is success */
          if( $this->_status == '0') {
            $result =  set_message( $adsType, __ ('Unpublished', FLICHE_VGALLERY ));
          }
          /** Check adsense delete action is success */
          if ($this->_del == '1') {
            $result = set_message( $adsType,__ ('Deleted', FLICHE_VGALLERY ));
          }
          /** Check adsense delete action is unsuccess */
          if ($this->_del == '1') {
            $result = set_message( $adsType, __ ('Not Deleted', FLICHE_VGALLERY ));
          }
          return $result;
        }        
...
switch($adminPage){
  case 'addgoogleadsense':
    require_once (FLICHE_VGALLERY_BASEDIR . DS . 'admin/views/videogoogleadsense/videoaddgoogleadsense.php');
    break;
  case 'googleadsense':
  default:
    require_once (FLICHE_VGALLERY_BASEDIR . DS . 'admin/views/videogoogleadsense/videogoogleadsense.php');
    break;
}
?>