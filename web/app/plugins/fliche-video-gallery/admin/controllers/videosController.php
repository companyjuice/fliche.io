...
/***************************************************************/
      ...
      public function getStatusMessage() {
          ...
          if (isset ( $this->_featured ) && $this->_featured == '1') {
            $result = set_message( $videoType, __ ( 'set as a Featured Video', FLICHE_VGALLERY ));
          }
          /** Check video unfeatured action is success */
          if( $this->_featured == '0') {
            $result =  set_message( $videoType, __ ( 'set as Unfeatured Video', FLICHE_VGALLERY ));
          }
          /** Check video publish action is success */
          if (isset ( $this->_status ) && $this->_status == '1') {
            $result = set_message( $videoType, __ ( 'Published', FLICHE_VGALLERY ));
          }
      ...
      public function get_message() {
        ...
        if ($this->_add == '1') {
          $result = set_message( $videoType, __ ( 'Added', FLICHE_VGALLERY ));
        }
        /** Check video update action is success */
        if($this->_update == '1') {
          $result = set_message( $videoType, __ ( 'Updated', FLICHE_VGALLERY ));
        } 
        /** Check video update action is fail */
        if($this->_update == '0' ) {
            $result = set_message( $videoType, __ ( 'Not Updated', FLICHE_VGALLERY ));
        }
        /** Check video delete action is success */
        if (isset ( $this->_del ) && $this->_del == '1') {
          $result = set_message( $videoType, __ ( 'Deleted', FLICHE_VGALLERY ));
        }
