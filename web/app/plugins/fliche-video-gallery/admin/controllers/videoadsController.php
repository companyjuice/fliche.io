  ...
  class VideoadController extends VideoadModel {      
      ...
      */
      ...
      public function get_message() {
        ...
        if ($this->_status == '1') {
          $adsresult = set_message( $type, __ ('Published', FLICHE_VGALLERY ));
        }
        /** Check video ad publish is unsuccess */
        if( $this->_status == '0') {
          $adsresult =  set_message( $type,__ ('Unpublished', FLICHE_VGALLERY ));
        }
        /** Check video ad add action is success */
        if ($this->_add == '1') {
          $adsresult = set_message( $type, __ ('Added', FLICHE_VGALLERY ));
        }      
        /** Check video ad update is success */
        if($this->_update == '1') {
          $adsresult = set_message( $type, __ ('Updated', FLICHE_VGALLERY ));
        } 
        /** Check video ad update is unsuccess */
        if($this->_update == '0' ) {
          $adsresult = set_message( $type, __ ('Not Updated', FLICHE_VGALLERY ));
        }        
        /** Check video ad delete is success */
        if ( $this->_del == '1') {
          $adsresult = set_message( $type, __ ( 'Deleted', FLICHE_VGALLERY ));
        }
        return $adsresult;
      }
...
switch ($adminPage) {  
    case 'newvideoad':
      require_once (FLICHE_VGALLERY_BASEDIR . DS . 'admin/views/videoads/addvideoads.php');
      break;
    case 'videoads':
    default:
      require_once (FLICHE_VGALLERY_BASEDIR . DS . 'admin/views/videoads/videoads.php');
      break;
}
?>    