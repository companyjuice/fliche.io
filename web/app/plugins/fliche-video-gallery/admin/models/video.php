...
if (! class_exists ( 'VideoSubModel' )) {
  ...
  class VideoSubModel {
    ...
    public function get_videodata($searchValue, $searchBtn, $order, $orderDirection) {
      ...
      $query = 'SELECT DISTINCT ( a.vid ) FROM ' . $this->_videotable . ' a  LEFT JOIN ' . $wpdb->users . ' u ON u.ID=a.member_id  LEFT JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_med2play p  ON p.media_id=a.vid LEFT JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_playlist pl ON pl.pid=p.playlist_id '; //WHERE pl.is_publish=1 OR a.publish=1 
      ...
      $sqlQuery = 'SELECT DISTINCT ( a.vid ),a.*,u.display_name FROM ' . $this->_videotable . ' a  LEFT JOIN ' . $wpdb->users . ' u ON u.ID=a.member_id LEFT JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_med2play p ON p.media_id=a.vid LEFT JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_playlist pl ON pl.pid=p.playlist_id '; //WHERE pl.is_publish=1  OR a.publish=1 
      if (isset ( $searchBtn )) {
  ...
  class VideoPageSubModel extends VideoSubModel {
    ...
    public function video_count($searchValue, $searchBtn) {
      ...
      $sql = 'SELECT DISTINCT ( a.vid ) FROM ' . $this->_videotable . ' a
         ...
         LEFT JOIN ' . $this->_wpdb->prefix . 'hdflvvideoshare_playlist pl ON pl.pid=p.playlist_id '; // WHERE pl.is_publish=1  OR a.publish=1
