 ...
 * @author user
 */
class Widget_FlicheVideoCategory_init extends WP_Widget {      
    /**
     * This function is used to initialize Category widget
     */
    function __construct() {
        /** Store category widget class name, description in an array */
        $widgetOpts = array ( 'classname' => 'Widget_FlicheVideoCategory_init ', 'description' => 'Fliche Video Categories' );
        #$this->WP_Widget ( 'Widget_FlicheVideoCategory_init', 'Fliche Video Category', $widgetOpts );
        parent::__construct ( 'Widget_FlicheVideoCategory_init', 'Fliche Video Category', $widgetOpts );
    }
    ...
    /**
     * Function to display the catgoery list as a link
     * 
     * @param unknown $args
     * @param unknown $instance
     */
    function widget($args, $instanceCat) {
        ...
        if (! empty ( $features )) {
            ...
            $div  .= '<li>' . __ ( 'No Categories', FLICHE_VGALLERY ) . '</li>';
        }
        /** End category list */ 
        if (($show < $countCategories)) {
            $div  .= '<li><div class="right video-more"><a href="' . $more_videos_link . '">' . __ ( 'More Categories', FLICHE_VGALLERY ) . ' &#187;</a></div></li>';
        }
        $div    .= '</ul></div>';
...
add_action ( 'widgets_init', create_function ( '', 'return register_widget("Widget_FlicheVideoCategory_init" );' ) ); 
?>