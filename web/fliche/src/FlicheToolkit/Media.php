<?php
    ...
    class Media extends MediaParser
    {
         ...
         * @param Config $config A FlicheToolkit Config object
...
//          check the requested class exists
            $class_name = '\\FlicheToolkit\\'.$default_class_name.(empty($format) === false ? '_'.ucfirst(strtolower($format)) : '');
            if(class_exists($class_name) === false)
            {
                $requested_class_name = $class_name;
                $class_name = '\\FlicheToolkit\\'.$default_class_name;
                if(class_exists($class_name) === false)
                {
...
//          check that it extends from the base Format class.
            if($class_name !== '\\FlicheToolkit\\Format' && is_subclass_of($class_name, '\\FlicheToolkit\\Format') === false)
            {
                throw new \InvalidArgumentException('The class "'.$class_name.'" is not a subclass of \\FlicheToolkit\\Format.');
            }
        ...
        public function setMediaPath($media_file_path)
        {
            ...
            {
                throw new \InvalidArgumentException('The file "'.$media_file_path.'" cannot be found in \\FlicheToolkit\\Media::__construct.');
            }
            else if(is_readable($real_file_path) === false)
            {
                throw new \InvalidArgumentException('The file "'.$media_file_path.'" is not readable in \\FlicheToolkit\\Media::__construct.');
            }
...
//          check the split by
            ...
            {
                throw new \InvalidArgumentException('The split by value is empty, in \\FlicheToolkit\\'.get_class($this).'::split');
            }
...
//              we check to see if we have a timecode object, if we do then we are spliting at exact points
                if(is_object($split_by[0]) === true)
                {
                    ...
                    {
                        if(get_class($timecode) !== 'FlicheToolkit\Timecode')
                        {
                            throw new \InvalidArgumentException('The split by timecode specified in index '.$key.' is not a \\FlicheToolkit\\Timecode object.');
                        }
...
//          anything else is treated as an integer of which each split is the same length.
            else 
            {
                if($split_by < 1)
                {
                    throw new \InvalidArgumentException('The split by value must be >= 1, in \\FlicheToolkit\\'.get_class($this).'::split');
                }
...
//          check time delta
            if($time_delta < 0)
            {
                throw new \InvalidArgumentException('The time delta specified "'.$time_delta.'", in \\FlicheToolkit\\'.get_class($this).'::split must be >= 0');
            }
...
//          check the directory that contains the output list is writeable
            if(empty($output_list_path) === false)
            {
                ...
                {
                    throw new \InvalidArgumentException('The directory for the output list file "'.$output_list_path.'" does not exist, in \\FlicheToolkit\\'.get_class($this).'::split');
                }
                else if(is_writeable($output_list_dir) === false)
                {
                    throw new \InvalidArgumentException('The directory for the output list file "'.$output_list_path.'" is not writeable, in \\FlicheToolkit\\'.get_class($this).'::split');
                }
...
//          if a callback has been supplied then the process becomes blocking and must be set.      
        ...
        {
            $class = 'FlicheToolkit\MultiOutput'; // prevents unneccesary autoload.
         ...
         *  of the generated file, otherwise if a FlicheToolkit\MultiOutput object is given then we treat the output
         *  as multiple output.
