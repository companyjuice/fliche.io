...
//          add default input/output commands
            ...
            {
                throw new \InvalidArgumentException('Unrecognised input/output type "'.$input_output_type.'" set in \\FlicheToolkit\\Format::__construct');
            }
...
//          check the requested class exists
            $class_name = '\\FlicheToolkit\\'.$fallback_format_class.(empty($format) === false ? '_'.ucfirst(strtolower($format)) : '');
            if(class_exists($class_name) === false)
            {
                ...
                $class_name = '\\FlicheToolkit\\'.$fallback_format_class;
                if(class_exists($class_name) === false)
                {
...
//          check that it extends from the base Format class.
            ...
            {
                throw new \LogicException('The class "'.$class_name.'" is not a subclass of \\FlicheToolkit\\Format.');
            }
        ...
        /**
         * Sets the FlicheToolkit\Media object into the format so that format object can modify the Media object.     
         *
         * @access public
         * @author Oliver Lillie
         * @param FlicheToolkit\Media $media 
         * @return FlicheToolkit\Format Returns the current object.
         */
...
//          validate input
            ...
            throw new \InvalidArgumentException('Unrecognised format "'.$format.'" set in \\FlicheToolkit\\'.get_class($this).'::setFormat');
        }
        ...
        public function setPresetOptionsFile($preset_file_path)
        {
            ...
            {
                throw new \InvalidArgumentException('Preset options file "'.$preset_file_path.'" set in \\FlicheToolkit\\'.get_class($this).'::setPresetOptionsFile does not exist.');
            }
            else if(is_readable($preset_file_path) === false)
            {
                throw new \InvalidArgumentException('Preset preset options file "'.$preset_file_path.'" set in \\FlicheToolkit\\'.get_class($this).'::setPresetOptionsFile is not readable.');
            }
        ...
        public function setStrictness($strictness)
        {
            ...
            throw new \InvalidArgumentException('Unrecognised strictness "'.$strictness.'" set in \\FlicheToolkit\\'.get_class($this).'::setStrictness');
        }
         ...
         * @return FlicheToolkit\Format Returns the current object.
         * @throws \InvalidArgumentException If the format requested is "segment". This is a special ffmpeg format to split a file, however FlicheToolkit
...
//              check to see if segmenting has been requested. If it has warn of the Media::split function instead.
                if($format === 'segment')
                {
                    throw new \InvalidArgumentException('You cannot set the format to segment, please use instead the function \\FlicheToolkit\\Media::segment.');
                }
