...
//          validate the video codecs that are available from ffmpeg.
            ...
            {
                throw new \InvalidArgumentException('Unrecognised video codec "'.$video_codec.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoCodec');
            }
...
//          now check the class settings to see if restricted codecs have been set and have to be obeys
            if($this->_restricted_video_codecs !== null)
            {
                ...
                {
                    throw new \LogicException('The video codec "'.$video_codec.'" cannot be set in \\FlicheToolkit\\'.get_class($this).'::setVideoCodec. Please select one of the following codecs: '.implode(', ', $this->_restricted_video_codecs));
                }
            }
...
//              if we have the same as source dimensions...
            ...
            {
                throw new Exception('Unrecognised width dimension "'.$width.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoDimensions');
            }
            if(empty($height) === true || $height <= 0)
            {
                throw new Exception('Unrecognised height dimension "'.$height.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoDimensions');
            }
        ...
        public function setVideoScale($width, $height)
        {
            ...
            if($width <= 0)
            {
                throw new Exception('Unrecognised width dimension "'.$width.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoScale');
            }
            if($height <= 0)
            {
                throw new Exception('Unrecognised height dimension "'.$height.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoScale');
            }
...
//          if width and height is set then we must use the scale filter instead of dimensions
        ...
        public function setVideoAspectRatio($aspect_ratio, $auto_adjust_dimensions=false)
        {
            ...
            {
                throw new Exception('Unrecognised aspect ratio "'.$aspect_ratio.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoAspectRatio');
            }
        ...
        public function setVideoFrameRate($frame_rate)
        {
            ...
            if($frame_rate < 1)
            {
                throw new Exception('Unrecognised frame rate "'.$frame_rate.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoFrameRate');
            }
...
//          now check the class settings to see if restricted codecs have been set and have to be obeys
            if($this->_restricted_video_frame_rates !== null)
            {
                ...
                {
                    throw new Exception('The frame rate "'.$frame_rate.'" cannot be set in \\FlicheToolkit\\'.get_class($this).'::setVideoFrameRate. Please select one of the following frame rates: '.implode(', ', $this->_restricted_video_frame_rates));
                }
            }
        ...
        public function setVideoMaxFrames($max_frame_count)
        {
            ...
            {
                throw new Exception('Unrecognised max frame count "'.$max_frame_count.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoFrameRate');
            }
...
//          now check the class settings to see if restricted codecs have been set and have to be obeys
            if($this->_restricted_video_bitrates !== null)
            {
                ...
                {
                    throw new Exception('The bitrate "'.$bitrate.'" cannot be set in \\FlicheToolkit\\'.get_class($this).'::setVideoBitrate. Please select one of the following bitrates: '.implode(', ', $this->_restricted_video_bitrates));
                }
            }
            ...
            //throw new Exception('Unrecognised video bitrate "'.$bitrate.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoBitrate');
        }
...
//          now check the class settings to see if restricted pixel formats have been set and have to be obeyed
            if($this->_restricted_video_pixel_formats !== null)
            {
                ...
                {
                    throw new Exception('The video pixel format "'.$pixel_format.'" cannot be set in \\FlicheToolkit\\'.get_class($this).'::setVideoPixelFormat. Please select one of the following pixel formats: '.implode(', ', $this->_restricted_video_pixel_formats));
                }
            }
            ...
            throw new Exception('Unrecognised pixel format "'.$pixel_format.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoPixelFormat');
        }
...
//          interpret quality into ffmpeg value
            ...
            {
                throw new Exception('Unrecognised quality "'.$quality.'" set in \\FlicheToolkit\\'.get_class($this).'::setQuality');
            }
...
//          90 is the same as -270, etc.
            ...
            {
                throw new Exception('Unrecognised rotation "'.$rotation.'" set in \\FlicheToolkit\\'.get_class($this).'::setVideoRotation');
            }
